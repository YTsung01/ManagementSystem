...
<div class="container-xl mt-5">
	<form 
		action="/ManagementSystem/app/takeOff/update/${form.formId}"
		method="post" class="border rounded mx-auto p-4" id="takeOffUpdate">
		<div class="row">
			<!-- 左側 -->
			...
			<div class="row align-items-center pe-4 mb-3">
				...
				<div class="col-12 col-md-8">
					<label> <input type="radio" name="takeOffType"
						id="overtimeType1" required value="1"
						${overTime.overtimeType eq '1' ? 'checked' : ''} /> 加班費
					</label> <label> <input type="radio" name="takeOffType"
			...
			<div class="row align-items-center pe-4 mb-3">
				...
				<div class="col-12 col-md-2 position-relative">
					<input type="datetime-local" name="startTime" id="startTime"
						class="form-control" style="width: auto;" value="<fmt:formatDate value="${takeOff.startTime}"
							pattern="yyyy-MM-dd'T'HH:mm" />"
				...
				<div class="col-12 col-md-2 position-relative">
					<input type="datetime-local" name="endTime" id="endTime"
						...
						max="2023-12-31" value="<fmt:formatDate value="${takeOff.endTime}"
							pattern="yyyy-MM-dd'T'HH:mm" />" >
					<span id="takeOffEndError" style="color: red;"></span>
					<!-- 控制日期最大最小值 -->
				</div>
			</div>
			...
			<div class="row align-items-center pe-4 mb-5">
				...
				<div class="col-12 col-md-2 position-relative">
					...
					<input type="hidden" name="calculatedtakeOffHour"
						...
						value="${ takeoffHour.applyHour }" required readonly>
					<div
				...
				<span class="col-12 col-md-1  text-nowrap " id="takeoffHourStartError"
			...
			<div class="row  pe-4 mb-5 mt-3">
				...
				<div class="col-12 col-md-10 mb-4">
					<textarea name="reason" id="reason" class="form-control" rows="5"required ">${takeOff.reason}</textarea>
				</div>
			</div>
			...
			<div>
				<input name="ttakeOffFormId" id="takeOffFormId" type="hidden"
					value="${takeoffHourformId}" />
			</div>
...
<script>
        $(document).ready(function () {
            ...
            $("#searchDayoff").click(function () {
                ...
                var remainingHours = ${totaltakeoffHour}; 
...
$(document).ready(function () {
    $('#calculatebtn').on('click', function() {
        ...
        if (!startDateValue || !endDateValue) {
            // 若未選擇日期，顯示提示信息
           $('#takeOffStartError').text('請選擇加班日期');
            return;
        }else {
            $('#takeOffStartError').text(''); // 清除錯誤信息
        }
        ...
        if (startDate >= endDate) {
            $('#takeOffStartError').text('加班開始日期必須小於加班結束日期');
            return;
        } else {
            $('#takeOffStartError').text(''); // 清除錯誤信息
        }
        ...
        let calculatedResult = diffHours;
 		if (calculatedResult < 1) {
        	$('#takeOffStartError').text('加班時數最小單位為1小時，請重新確認'); return;
        } else {
            $('#takeOffStartError').text(''); // 清除錯誤信息
        }
        ...
        if (diffHours > 4  || !Number.isInteger(diffHours)) {
        	 $('#takeOffStartError').text('加班時數不可大於4小時，請重新確認');
            return;
        }
 		
        if (diffHours < 0  || !Number.isInteger(diffHours)) {
       	 $('#takeOffStartError').text('加班時數不可為空，請重新確認');
           return;
       }
 		
        // 將計算結果丟給隱藏的 input 元素
        $('#calculatedTakeOffHour').val(calculatedResult);

        // 更新顯示在 readonly 的 input 元素中
        $('#takeOffHour').val(calculatedResult);
